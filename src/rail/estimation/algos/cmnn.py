...
class CMNNInformer(CatInformer):
    ...
    interactive_function = "cmnn_informer"
    ...
    config_options.update(
        ...
        nondetect_replace=Param(
            bool,
            False,
            msg="set to True to replace non-detects,"
    ...
    def run(self):
        ...
        col_data, col_err = _computecolordata(
            training_data, self.config.bands, self.config.err_bands
        )

        self.model = dict(
            train_color=col_data,
            train_err=col_err,
            truez=specz,
...
class CMNNEstimator(CatEstimator):
    ...
    interactive_function = "cmnn_estimator"
    ...
    config_options.update(
        ...
        bad_redshift_val=Param(float, 99.0, msg="redshift to assign bad redshifts"),
        bad_redshift_err=Param(
            float, 10.0, msg="Gauss error width to assign to bad redshifts"
        ),
    )
    ...
    def _process_chunk(self, start, end, data, first):
        ...
        test_color, test_err = _computecolordata(
        ...
        for ii in range(num_gals):
            MahalanobisDistance = np.nansum(
                (test_color[ii] - self.train_color) ** 2 / test_err[ii] ** 2,
                axis=1,
                dtype="float",
            )
            ...
            if self.nondet_choice:
                ...
                deg_of_freedom = np.nansum(
                    (test_color[ii] ** 2 + self.train_color**2 + 1.0)
                    / (test_color[ii] ** 2 + self.train_color**2 + 1.0),
                    axis=1,
                    dtype="int",
                )
            ...
            if len(index) >= self.config.min_n:
                ...
                if self.config.selection_mode == 1:
                    tx = np.where(
                        MahalanobisDistance[index]
                        == np.nanmin(MahalanobisDistance[index])
                    )[0]
                    if len(tx) == 1:
                        rval = tx[0]
                ...
                index2 = np.where(
                    (threshold > 0.00010) & (MahalanobisDistance > 0.00010)
                )[0]
                if len(index2 > self.config.min_n):
                    ...
                    new_MD = np.asarray(
                        tempMD[sx[0 : self.config.min_n]], dtype="float"
                    )
                    new_TZ = np.asarray(
                        tempTZ[sx[0 : self.config.min_n]], dtype="float"
                    )
                    if self.nondet_choice:
                        ...
                        new_DOF = np.asarray(
                            tmpDOF[sx[0 : self.config.min_n]], dtype="float"
                        )
                        lim_DOF = new_DOF[-1]
                        del new_DOF
                    ...
                    if self.config.selection_mode == 2:
                        ...
                        weights = tweights / np.sum(tweights)
                        cx = rng.choice(
                            self.config.min_n, size=1, replace=False, p=weights
                        )[0]
                        out_pz = new_TZ[cx]
        ...
        ens = qp.Ensemble(
            qp.stats.norm,
            data=dict(
                loc=np.expand_dims(chunk_pz, -1), scale=np.expand_dims(chunk_pze, -1)
            ),
        )
